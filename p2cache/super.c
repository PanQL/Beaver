...
int ovl_rebuild_dentry_cache(struct super_block *sb, void* pmem_addr)
{
	...
	addr = pmem_addr;
	pr_info("%s: sb->s_user_ns=0x%px\n", __func__, sb->s_user_ns);
	//getrawmonotonic(&begin);
	for(i = 0; i < 10000000; i++) {
		memcpy(log_buff, addr, 512);
		dentry = d_alloc_name(sb->s_root, log_buff);
		inode = pcache_get_inode(sb->s_user_ns, sb,
				NULL, S_IFREG | 0755, 0, 0);
		d_add(dentry, inode);
		addr += 512;
	}
	//getrawmonotonic(&end);
...
static void ovl_put_super(struct super_block *sb)
{
	...
	kfree(sbi->zeroed_page);
	rcu_barrier();
	pr_info("%s: dax holder=0x%px.\n", __func__, sbi);
	fs_put_dax(sbi->s_dax_dev, sbi);
	rcu_barrier();
...
void ovl_evict_inode(struct inode *inode)
{
	...
	XA_STATE(xas, &sih->dram_pages, 0);
	void *kmem_dram;

	xas_for_each(&xas, kmem_dram, LONG_MAX) {
		if (kmem_dram != NO_PAGE) {
			free_pages(kmem_dram, 0);
		}
	}
	...
	struct ovl_sb_info *sbi)
{
	...
	pr_info("%s: dax holder=0x%px.\n", __func__, sbi);
	dax_dev = fs_dax_get_by_bdev(sb->s_bdev, &startoff,
	...
	num_blocks = dax_direct_access(dax_dev, 0, LONG_MAX/PAGE_SIZE, DAX_ACCESS, &virt_addr, &__pfn_t);
...
static int ovl_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	root_inode = pcache_get_inode(sb->s_user_ns, sb,
	...
	/*if(sbi->enable_dram_page) {
		sbi->dram_pool = vzalloc_node(DRAM_CAPACITY_BYTES, 0);
		BUG_ON(!sbi->dram_pool);
		pr_info("dram_pool: %lx\n", sbi->dram_pool);



		unsigned long per_cpu_pages = DRAM_CAPACITY_BYTES / PAGE_SIZE / sbi->cpus;
		for(i = 0; i < sbi->cpus; i++)
			sbi->dram_alloc[i] = per_cpu_pages * i;
		for(i = 0; i < sbi->cpus - 1; i++)
			sbi->dram_sentinel[i] = sbi->dram_alloc[i+1] - 1;
		sbi->dram_sentinel[sbi->cpus - 1] = DRAM_CAPACITY_BYTES / PAGE_SIZE - 1;
		for(i = 0; i < sbi->cpus; i++)
			pr_info("cpu %d: dram_alloc: %lu dram_sentinel: %lu\n",
				i, sbi->dram_alloc[i], sbi->dram_sentinel[i]);
	}*/
