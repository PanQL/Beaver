...
static int
ovl_mknod(struct user_namespace *userns, struct inode *dir,
	    struct dentry *dentry, umode_t mode, dev_t dev)
{
	...
	inode = pcache_get_inode(userns, dir->i_sb, dir, mode, dev, 0);
	int error = -ENOSPC;
...
static int ovl_mkdir(struct user_namespace *userns, struct inode *dir,
		       struct dentry *dentry, umode_t mode)
{
	int retval; 
	// pr_info("%s", __func__);
	retval = ovl_mknod(userns, dir, dentry, mode | S_IFDIR, 0);
	if (!retval)
		inc_nlink(dir);
	// busy_loop();
	return retval;
...
static int ovl_create(struct user_namespace *userns, struct inode *dir,
			struct dentry *dentry, umode_t mode, bool excl)
{
	// pr_info("%s", __func__);
	return ovl_mknod(userns, dir, dentry, mode | S_IFREG, 0);
}
...
int ovl_rename(struct user_namespace *userns, struct inode *old_dir,
		struct dentry *old_dentry, struct inode *new_dir,
		struct dentry *new_dentry, unsigned int flags)
{
	int rc;
	// pr_info("%s", __func__);
	rc = simple_rename(userns, old_dir, old_dentry,
...
int ovl_setattr(struct user_namespace *userns,
		struct dentry *dentry, struct iattr *iattr)
{
	int ret;
	// pr_info("%s", __func__);
	ret = simple_setattr(userns, dentry, iattr);
	// busy_loop();
	return ret;
}

static int ovl_symlink(struct user_namespace *userns,
		struct inode * dir, struct dentry *dentry, const char * symname)
{
	...
	inode = pcache_get_inode(userns, dir->i_sb, dir, S_IFLNK|S_IRWXUGO, 0, 0);
	if (inode) {
		int l = strlen(symname)+1;
		error = page_symlink(inode, symname, l);
		if (!error) {
