...
int nova_getattr(struct user_namespace *userns, const struct path *path,
...
int nova_notify_change(struct user_namespace *userns,
			struct dentry *dentry, struct iattr *attr)
{
	...
	ret = setattr_prepare(userns, dentry, attr);
	if (ret)
		goto out;
...
{
	int ret;
	struct nova_sb_info *sb_info = mapping->host->i_sb->s_fs_info;
	INIT_TIMING(wp_time);

	NOVA_START_TIMING(write_pages_t, wp_time);
	ret = dax_writeback_mapping_range(mapping,
			sb_info->s_dax_dev, wbc);
