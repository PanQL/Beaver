	...
	struct nova_inode *pi, struct nova_inode_info_header *sih)
{
	...
	rc = copy_mc_to_kernel(&fake_pi, pi,
...
static inline u64 alter_log_page(struct super_block *sb, u64 curr)
{
	...
	curr = BLOCK_OFF(curr);
	curr_page = (struct nova_inode_log_page *)nova_get_block(sb, curr);
	rc = copy_mc_to_kernel(&next,
			       &curr_page->page_tail.alter_page,
			       sizeof(u64));
	if (rc)
		return rc;
...
int nova_iomap_begin(struct inode *inode, loff_t offset, loff_t length,
	unsigned int flags, struct iomap *iomap, struct iomap *srcmap, bool taking_lock);
