...
static int
nova_dax_notify_failure(struct dax_device *dax_dev, u64 offset,
			u64 len, int mf_flags)
{
	pr_warn("memory failure notifying in novafs not implemented!\n");
	return -EIO;
}

const struct dax_holder_operations nova_dax_holder_operations = {
	.notify_failure		= nova_dax_notify_failure,
};
	...
	struct nova_sb_info *sbi)
{
	...
	dax_dev = fs_dax_get_by_bdev(sb->s_bdev, &startoff,
				     sbi, &nova_dax_holder_operations);
	...
	size = dax_direct_access(sbi->s_dax_dev, 0, LONG_MAX/PAGE_SIZE, DAX_ACCESS,
